...
#define Nthreads 8//used lscpu to find that my system had 8 cores 
#define Nprocs 8//will experiment on this
// will experiment with cpu affinity as well
#define Nruns 1
...
void* job(void* jargs);//job function(in mutlithreading) to calculate elements indexed from i to j
void proc_job(int* A,int*B,int* C,long long int i,long long int j,struct Args args,volatile int* go_flag);//analogous to job above for mulitprocessing; usage of volatile explained in report 

unsigned long long mean_time(unsigned long long* a){
...
int main(int argc, char *argv[])
{
	...
	/* My code */
	// Test run code 
	// multiple runs for the average run times 
	// interating for Nruns and reporting the corresponding speed up 
	
	unsigned long long s_t_times[Nruns];
	unsigned long long m_t_times[Nruns];
	unsigned long long m_p_times[Nruns];
	...
	fprintf(stdout,"NRUNS:%d | Nthreads:%d | Nprocs:%d \n",Nruns,Nthreads,Nprocs);
	for(int i=0;i<Nruns;i++){
	s_t_times[i]= single_thread_mm(A,B,args);
	m_t_times[i]= multi_thread_mm(A,B,args);
	m_p_times[i]= multi_process_mm(args);
	}

	time_single = mean_time(s_t_times);
	time_multi_thread = mean_time(m_t_times);
	time_multi_process = mean_time(m_p_times);
...
void proc_job(int* A,int*B,int*C,long long int i, long long int j,struct Args args,volatile int* go_flag){
	int x,y;
	while(!*go_flag)
		continue;
