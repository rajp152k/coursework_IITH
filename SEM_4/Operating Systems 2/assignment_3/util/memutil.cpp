...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < op_cnt; i++) {
		...
		case OP_MAPREAD:{
				...
				// [x] memory map the devicemem's kernel buffer into user-space segment.
				// TODO
				dev_mem =
				...
				if (msg_len == 0) {
					cerr <<
					    "nothing to compare what is being read"
					    << endl;
					cerr <<
					    "reading allthe characters into a buffer character to still monitor page faults"
					    << endl;
					for (int i = 0; i < len; i++ 0) {
						buffer = *(dev_mem + i);
					}
				} else {	//can compare the read data 
					for (int i = 0; i < len; i++) {
						if (*(msg + (i % msg_len)) !=
						    *(dev_mem + i)) {
							cerr <<
							    "the message doesn't match to what is being read"
							    << endl;
						}
					}
				}
				...
				// [x] unmap the devicemem's kernel buffer.
				// TODO
