...
class EuropeanOptions:
    ...
    def gen_sim(self, granularity: int, num_sim: int, antithetic_variate = False):
        ...
        if antithetic_variate:
            St_antithetic = self.S*np.exp((self.mu - 0.5*self.sigma**2)*self.T - self.sigma*sims)
...
class Experiment:
    def __init__(self,pricer,all = True,exp_dir = "./experiments"):
    ...
    def exp_granularity(self,pricer,num_sim = 1000):
        ...
        for N in tqdm(Ns):
            call_price,call_std,call_ste = pricer.monte_carlo('call',N,num_sim)
            put_price,put_std,put_ste = pricer.monte_carlo('put',N,num_sim)
    ...
    def exp_num_sim(self, pricer,granularity=1000):
        call_cols = ['call_price','std','ste','black_scholes_call']
        put_cols = ['put_price','std','ste','black_scholes_put']
        ...
        for M in tqdm(Ms):
            call_price,call_std,call_ste = pricer.monte_carlo('call',granularity,M)
            put_price,put_std,put_ste = pricer.monte_carlo('put',granularity,M)
    ...
    def exp_antithetic(self,pricer,granularity=1000):
        ...
        for M in tqdm(Ms):
            call_price,call_std,_ = pricer.monte_carlo('call',granularity,M)
            call_a_price,call_a_std,_ = pricer.monte_carlo('call',granularity,M,True)

            put_price,put_std,_ = pricer.monte_carlo('put',granularity,M)
            put_a_price,put_a_std,_ = pricer.monte_carlo('put',granularity,M,True)
...
if (__name__ == '__main__'):
    NUM_SIMULATIONS = 1000  # num trials
    granularity = 1000       # T/dt


    # defaults
    r = 0.04                # annual risk-free interest rate
    sigma = 0.25                # annual fractional volatility
    T = 0.25                # time to maturity in years
    S = 100                 # initial stock price (pays no dividend)
    K = 105                 # strike price

    price_EO = EuropeanOptions(S,K,T,r,sigma)

    TEST_ALL = True
    # EXPERIMENTS
    exp  = Experiment(price_EO,all = TEST_ALL)
